...
type ColumnType = ColumnDef<ResourceInterface, unknown>;
...
export function ResourceListPage(props: ResourceListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getResources({
        relations: ['platform', 'download.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ResourceInterface>>(
    () => `/resources?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('platform', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('download', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'download',
          header: 'Download',
          accessorKey: 'download',
          cell: ({ row: { original: record } }: any) => record?._count?.download,
        }
      : null,
