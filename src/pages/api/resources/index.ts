...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getResources() {
    ...
    const response = await prisma.resource
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'resource'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createResource() {
    ...
    if (body?.download?.length > 0) {
      const create_download = body.download;
      body.download = {
        create: create_download,
      };
    } else {
      delete body.download;
    }
